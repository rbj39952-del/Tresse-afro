...
export default function AdminPage() {
  ...
  const handleAddService = (e: React.FormEvent) => {
    ...
    setFormData({ name: '', type_id: '', city: '', price_range: 'medium', price_min: 0, price_max: 0, duration_minutes: 0, salon_name: '', salon_phone: '', instagram_url: '', whatsapp_url: '', image_url: '', description: '' })
  ...
  const handleAddType = (e: React.FormEvent) => {
    ...
    const type: HairstyleType = { id: newType.toLowerCase().replace(/\s+/g, '-'), name: newType }
    setHairstyleTypes([...hairstyleTypes, type])
    setNewType('')
    setShowAddType(false)
  }

  const typeNameMap = useMemo(() => {
    return hairstyleTypes.reduce((acc, type) => ({ ...acc, [type.id]: type.name }), {} as Record<string, string>)
  }, [hairstyleTypes])
            ...
            <div className="border border-border rounded-2xl p-8">
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                <div>
                  ...
                  <input type="password" value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} className="input w-full" placeholder="Entrez le mot de passe admin" />
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => { setShowAddService(!showAddService); setEditingService(null); setFormData({ name: '', type_id: '', city: '', price_range: 'medium', price_min: 0, price_max: 0, duration_minutes: 0, salon_name: '', salon_phone: '', instagram_url: '', whatsapp_url: '', image_url: '', description: '' }) }} className="btn btn-primary flex items-center gap-2">
